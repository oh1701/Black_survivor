...
class item_in(context: Context) {
    ...
    fun falsearray(j: Int) {
        ...
        if (falselist[j].name1 == "모노호시자오") {
            te("모노호시자오", "공격력 +80, 생명력 흡수 +25%", "[제작] 무라마사 + 정교한 도면", "전설")
            down("무라마사", "정교한 도면", "", "", "")
        }
        ...
        if (falselist[j].name1 == "95식 자동소총") {
            te("95식 자동소총", "장탄 수: 30발, 공격력 +77, 시야 +1.5\n 기본 공격 사거리 +1", "[제작]\nAK-47 + 강철", "영웅")
            down("AK-47", "강철", "", "", "")
        }
        ...
        if (falselist[j].name1 == "페일노트") {
            te("페일노트", "공격력 +105, 공격속도 +15%, 생명력 흡수 +20%", "[제작]\nVF 혈액 샘플 + 강궁", "전설")
            up("", "", "", "", "")
            down("VF 혈액 샘플", "강궁", "", "", "")
        }
        ...
        if (falselist[j].name1 == "틴 스피릿") {
            te("틴 스피릿", "공격력 +10, 기본 공격 사거리 +1, 공격속도 +40%, 기본 공격 추가 피해 +25", "[제작]\n야생마 + 진신사리", "영웅")
            up("", "", "", "", "")
            down("야생마", "진신사리", "", "", "")
        }
        ...
        if (falselist[j].name1 == "기사의 신조") {
            te("기사의 신조", "공격력 +30, 방어력 +20, 체력 재생 +1.2, 이동속도 -0.03, 치명타 피해량 +25%", "[제작] 강철 방패 + 분대장 완장\n보라색 항공 보급", "영웅")
            up("", "", "", "", "")
            down("강철 방패", "분대장 완장", "", "", "")
        }
        ...
        if (falselist[j].name1 == "매버릭 러너") {
            te("매버릭 러너", "공격력 -8, 이동속도 +0.25, 비전투중 이동속도 +0.3", "[제작] 힐리스 + 전자 부품\n파란색 항공 보급", "희귀")
            up("", "", "", "", "")
            down("힐리스", "전자 부품", "", "", "")
        }
        ...
        if (falselist[j].name1 == "스크램블 에그") {
            te("스크램블 에그", "체력 재생 +600", "[제작] 달걀 + 뜨거운 오일\n파란색 항공 보급", "희귀")
            up("", "", "", "", "")
            down("달걀", "뜨거운 오일", "", "", "")
        }
        if (falselist[j].name1 == "초코칩 쿠키") {
        ...
        if (falselist[j].name1 == "포스 코어") {
            ...
            im.setOnClickListener() {
                if (dd == 0) {
                    ...
                    im.text = ("상위 아이템2 (터치 시 상위 아이템3로 변경)")
                    dd += 1
                } else if(dd == 1){
                    for (j in falselist.indices) {
                        if (falselist[j].name1 == "아르기로톡소스") {
        ...
        if (falselist[j].name1 == "정찰 드론") {
            te("정찰 드론", "지속 시간동안 효과 범위의 공중 시야를 밝혀줍니다. 투명 및 은신은 감지하지 못합니다.", "[제작] 감시 카메라 + 방전 전지", "고급")
            up("EMP 드론", "", "", "", "")
            down("감시 카메라", "방전 전지", "", "", "")
        }
        if (falselist[j].name1 == "EMP 드론") {
            te("EMP 드론", "EMP 필드 생성 후 지속 시간동안 범위 내 모든 트랩/포탑을 무력화 및 공중 시야를 밝혀줍니다.",
                "[제작] 정찰 드론 + 화약", "희귀")
            down("정찰 드론", "화약", "", "", "")
        }
    }//함수 닫히는곳
