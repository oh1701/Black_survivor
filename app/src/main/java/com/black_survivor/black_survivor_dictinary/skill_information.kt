...
class skill_information : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.hasExtra("로지")) {
            ...
            binding.rExeplan1.text =
                    "로지가 적이나 지면에 부착되고 3초 후에 폭발하는 폭탄을 발사합니다.\n" +
                            "폭탄을 적에게 붙였다면 이동속도가 20/25/30% 감소하고, 폭발할 때 피해를 입힙니다. 부착된 폭탄이 터지기 전에 대상에게 기본 공격을 5회 적중하면 즉시 터지며 대상 최대 체력의 4/8/12%만큼의 고정 피해를 추가로 입힙니다. 스킬을 적중하면 2회 공격으로 간주합니다.폭탄을 즉시 터트리면 셈텍스탄 Mk-II 스킬의 쿨다운이 45% 감소하고, 1초 동안 이동속도가 15% 증가합니다."
        ...
        if (intent.hasExtra("바바라")) {
            binding.skill1.setImageResource(R.drawable.barbara)
            binding.change.visibility = View.VISIBLE
            binding.texc.visibility = View.VISIBLE
