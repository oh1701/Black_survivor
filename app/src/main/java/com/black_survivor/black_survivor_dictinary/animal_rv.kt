...
class animal_rv(val animalItem:ArrayList<animal_Item_list>): RecyclerView.Adapter<animal_rv.ViewHolder>() {
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): animal_rv.ViewHolder {

        val view = LayoutInflater.from(parent.context).inflate(R.layout.animal, parent, false)
        var rnqns: Int = 0
        var qkRnrl: Int = -1
        var qkRnrl2:Int = -1
        var qkRnjtek:Int = -1
        val aview: View = LayoutInflater.from(parent.context).inflate(R.layout.animal_popup, null)
        ...
        val weck = view.findViewById<ImageView>(R.id.weckeline)
        ...
        val cview: View = LayoutInflater.from(parent.context).inflate(R.layout.activity_popup, null)
        val bview: View = LayoutInflater.from(parent.context).inflate(R.layout.activity_imsi, null)

        val ImageView1: ImageView = cview.findViewById(R.id.im1)

        val nam2: TextView = cview.findViewById(R.id.te1)

        val uptext1: TextView = cview.findViewById(R.id.imup_text1)
        val uptext2: TextView = cview.findViewById(R.id.imup_text2)
        val uptext3: TextView = cview.findViewById(R.id.imup_text3)
        val uptext4: TextView = cview.findViewById(R.id.imup_text4)
        val uptext5: TextView = cview.findViewById(R.id.imup_text5)
        val im: TextView = cview.findViewById(R.id.te4)

        val downtext1: TextView = cview.findViewById(R.id.imdown_text1)
        val downtext2: TextView = cview.findViewById(R.id.imdown_text2)
        val downtext3: TextView = cview.findViewById(R.id.imdown_text3)
        val downtext4: TextView = cview.findViewById(R.id.imdown_text4)
        val downtext5: TextView = cview.findViewById(R.id.imdown_text5)
        ...
        val grade: TextView = cview.findViewById(R.id.te2_1)
        val abil: TextView = cview.findViewById(R.id.te22_1)
        val exeplan: TextView = cview.findViewById(R.id.te3_1)
        ...
        fun falsearray(j:Int){

            if (falselist[j].name1 == "채찍") {
                te("채찍", "공격력 +12", "성당(3개), 학교(3개)의 상자", "일반")
                up("오랏줄", "철편", "", "", "")
            }
            if (falselist[j].name1 == "오랏줄") {
                te("오랏줄", "공격력 +24, 스킬 증폭 +6", "[제작] 채찍 + 마패", "고급")
                up("바람 채찍", "", "", "", "")
                down("채찍", "마패", "", "", "")
            }
            if (falselist[j].name1 == "철편") {
                te("철편", "공격력 +30", "[제작] 채찍 + 면도칼", "고급")
                up("뇌룡편", "박력편", "", "", "")
                down("채찍", "면도칼", "", "", "")
            }
            if (falselist[j].name1 == "바람 채찍") {
            ...
            if (falselist[j].name1 == "박력편") {
                te("박력편", "공격력 +42\n" +
                        "공격 속도 +20%", "[제작] 철편 + 방전 전지", "희귀")
                up("플라즈마 윕", "", "", "", "")
                down("철편", "방전 전지", "", "", "")
            }
            if (falselist[j].name1 == "글레이프니르") {
                te("글레이프니르", "공격력 +58\n" +
                        "스킬 증폭 +40", "[제작] 바람 채찍 + 가시 발판", "영웅")
                up("", "", "", "", "")
                down("바람 채찍", "가시 발판", "", "", "")
            }
            if (falselist[j].name1 == "플라즈마 윕") {
                te("플라즈마 윕", "공격력 +60, 공격 속도 +40%\n" +
                        "시야 +2.3, 기본 공격 추가피해 +15", "[제작] 박력편 + 레이저 포인터", "영웅")
                up("", "", "", "", "")
                down("박력편", "레이저 포인터", "", "", "")
            }
            if (falselist[j].name1 == "혈화구절편") {
            ...
            if (falselist[j].name1 == "메스") {
                te("메스", "공격력 +18", "[제작] \n만년필 + 면도칼", "고급")
                up("스위스 아미 나이프", "", "", "", "")
            }

            if (falselist[j].name1 == "스위스 아미 나이프") {
                te("스위스 아미 나이프", "공격력 +23, 체력 +100", "[제작] \n메스 + 가위", "희귀")
                up("다마스커스 가시", "", "", "", "")
                down("메스","가위","","","")
            }

            if (falselist[j].name1 == "다마스커스 가시") {
                te("다마스커스 가시", "공격력 +66, 최대 체력 +250, 스킬 적중 시 치유 감소 -40%", "[제작] \n스위스 아미 나이프 + 가시 발판", "영웅")
                down("스위스 아미 나이프", "가시 발판", "", "", "")
            }
            if (falselist[j].name1 == "가위") {
            ...
            if (falselist[j].name1 == "식칼") {
                te("식칼", "공격력 +4" + "이동속도 +0.05", "단검 사용 시 기본 지급, 골목길(5개), 호텔(4개), 절(6개)의 상자\n,항구(4개) 멧돼지(사냥)", "일반")
            ...
            if (falselist[j].name1 == "카른웬난") {
                te("카른웬난", "공격력 +41, 이동속도 +0.1, 체력 재생 +1.2\n 쿨다운 감소 +12%, 기본 공격 추가 피해 +29", "[제작] \n 장미칼 + 성자의 유산", "영웅")
                down("장미칼", "성자의 유산", "", "", "")
            }
            if (falselist[j].name1 == "파산검") {
                te("파산검", "공격력 +35, 이동속도 +0.1 \n쿨다운 감소 +15%, 스킬 증폭 +30", "[제작] \n 장미칼 + 재", "영웅")
                down("장미칼", "재", "", "", "")
            }
            if (falselist[j].name1 == "초진동나이프") {
            ...
            if (falselist[j].name1 == "아론다이트") {
                te("아론 다이트", "공격력 +50, 공격속도 +30%\n 기본공격 추가피해 +25", "[제작] 마사무네 + 십자가", "영웅")
                down("마사무네", "십자가", "", "", "")
            }
            if (falselist[j].name1 == "엑스칼리버") {
                te("엑스칼리버", "공격력 +60, 최대체력 +360, 공격속도 +10%, 체력재생 +200%", "[제작] 보검 + 성배", "영웅")
                down("보검", "성배", "", "", "")
            }
            if (falselist[j].name1 == "모노호시자오") {
                te("모노호시자오", "공격력 +80, 생명력 흡수 +25%", "[제작] 무라마사 + 정교한 도면", "영웅")
                down("무라마사", "정교한 도면", "", "", "")
            }
            if (falselist[j].name1 == "호푸어드") {
                te("호푸어드", "공격력 +50, 최대체력 +300, 치명타 확률 +28%", "[제작] 보검 + 유리 조각", "영웅")
                down("보검", "유리 조각", "", "", "")
            }
            if (falselist[j].name1 == "레바테인") {
            ...
            if (falselist[j].name1 == "파라슈") {
                te("파라슈", "공격력 +115, 이동속도 +0.07, 쿨다운 감소 +10%", "[제작] 경량화 도끼 + 진신사리", "영웅")
                down("경량화 도끼", "진신사리", "", "", "")
            }
            if (falselist[j].name1 == "하르페") {
                te("하르페", "공격력 +105, 이동속도 +0.07, 스킬 증폭 +33, 스킬 공격 사거리+0.5, 스킬 적중 시 치유 감소 -40%", "[제작] 사신의 낫 + 백우선", "영웅")
                down("사신의 낫", "백우선", "", "", "")
            }
            if (falselist[j].name1 == "쌍칼") {
            ...
            if (falselist[j].name1 == "자웅일대검") {
                te("자웅일대검", "공격력 +60, 공격속도 +40%, 기본공격 추가피해 +8", "[제작] \n쌍칼 + 문스톤", "영웅")
                down("쌍칼", "문스톤", "", "", "")
            }
            if (falselist[j].name1 == "디오스쿠로이") {
                te("디오스쿠로이", "공격력 +48, 공격속도 +50%", "[제작] \n쌍칼 + 이온 전지", "영웅")
                down("쌍칼", "이온 전지", "", "", "")
            }
            if (falselist[j].name1 == "로이거 차르") {
                te("로이거 차르", "공격력 +38, 공격속도 +20%, 스킬 증폭 +36", "[제작] \n쌍칼 + 독약", "영웅")
                down("쌍칼", "독약", "", "", "")
            }
            if (falselist[j].name1 == "발터 PPK\u002dMagnum") {
            ...
            if (falselist[j].name1 == "더블 리볼버 SP") {
            ...
            if (falselist[j].name1 == "엘레강스") {
                te("엘레강스", "장탄 수: 6발, 공격력 +90, 이동속도 +0.2, 시야 +5", "[제작]\n FN57 + 먼지털이개", "영웅")
                down("FN57", "먼지털이개", "", "", "")
            }
            if (falselist[j].name1 == "일렉트론 블라스터") {
                te("일렉트론 블라스터", "장탄 수: 10발, 공격력 +61, 이동속도 +0.12\n기본 공격 추가 피해 +31", "[제작]\n 베레타 M92F + 이온 전지", "영웅")
                down("베레타 M92F", "이온 전지", "", "", "")
            }
            if (falselist[j].name1 == "매그넘\u002d보아") {
                te("매그넘\u002d보아", "장탄 수: 7발, 공격력 +90, 이동속도 +0.15\n시야 +2.5, 생명력 흡수 +22%", "[제작]\n 매그넘\u002d아나콘다 + 강철", "영웅")
                down("매그넘-아나콘다", "강철", "", "", "")
            }
            ...
            if (falselist[j].name1 == "페도로프 자동소총") {
            ...
            if (falselist[j].name1 == "개틀링 건") {
                te("개틀링 건", "장탄 수: 30발, 공격력 +20, 시야 +1.5\n 기본 공격 추가 피해 +11, 공격 속도 +10%", "[제작]\nSTG\u002d44 + 모터", "영웅")
                down("STG-44", "모터", "", "", "")
            }
            if (falselist[j].name1 == "AK\u002d12") {
                te("AK\u002d12", "장탄 수: 30발, 공격력 +89, 시야 +1.5\n 치명타 확률 20%", "[제작]\nAK-47 + 유리판", "영웅")
                down("AK-47", "유리판", "", "", "")
            }
            if (falselist[j].name1 == "XCR") {
                te("XCR", "장탄 수: 30발, 공격력 +63, 시야 +1.5\n 최대 체력 +330, 기본 공격 추가 피해 +10", "[제작]\nM16A1 + 탄창", "영웅")
                down("M16A1", "탄창", "", "", "")
            }
            if (falselist[j].name1 == "95식 자동소총") {
                te("95식 자동소총", "장탄 수: 30발, 공격력 +72, 시야 +1.5\n 기본 공격 사거리 +0.5", "[제작]\nAK-47 + 강철", "영웅")
                down("AK-47", "강철", "", "", "")
            }
            if (falselist[j].name1 == "아그니") {
                te("아그니", "장탄 수: 30발, 공격력 +55, 시야 +2.5\n 공격 속도 +33%, 기본 공격 추가 피해 +16", "[제작]\n개틀링 건 + 뜨거운 오일", "영웅")
                down("개틀링 건", "뜨거운 오일", "", "", "")
            }
            if (falselist[j].name1 == "화승총") {
            ...
            if (falselist[j].name1 == "Tac\u002d50") {
                te("Tac\u002d50", "장탄 수 : 2발, 공격력 +128, 시야 +2.5, 생명력 흡수 +20%", "[제작]\n 하푼건 + 정교한 도면", "영웅")
                down("하푼건", "정교한 도면", "", "", "")
            }
            if (falselist[j].name1 == "인터벤션") {
                te("인터벤션", "장탄 수 : 2발, 공격력 +120, 시야 +5, 기본 공격 사거리 +1.5", "[제작]\n 하푼건 + 망원 카메라", "영웅")
                down("하푼건", "망원 카메라", "", "", "")
            }
            ...
            if (falselist[j].name1 == "폴라리스") {
                te("폴라리스", "장탄 수 : 4발, 공격력 +110, 시야 +2, 공격속도 +35%", "[제작]\n 레일건 + 백색 가루", "영웅")
                down("레일건", "백색 가루", "", "", "")
            }

            if (falselist[j].name1 == "사사성광") {
                te("사사성광", "장탄 수 : 2발, 공격력 +100, 시야 +2.5, 이동속도 -0.03, 스킬증폭 +33%", "[제작]\n 문스톤 + 금교전", "전설")
                down("문스톤", "금교전", "", "", "")
            }
            if (falselist[j].name1 == "바늘") {
                te("바늘", "공격력 +3, 치명타 피해량 +25%", "사용 시 기본 지급\n" +
                        "골목길(6개), 호텔(5개), 병원(6개)의 상자", "일반")
                up("레이피어", "분대장 완장", "한방침", "다트", "독침")
            }
            if (falselist[j].name1 == "레이피어") {
                te("레이피어", "공격력 +16, 치명타 피해량 +25%", "[제작] \n바늘 + 철광성", "고급")
                up("매화검", "활빈검", "주와이외즈", "", "")
                down("바늘", "철광석", "", "", "")
            }
            if (falselist[j].name1 == "매화검") {
                te("매화검", "공격력 +121, 치명타 확률 +10%, 치명타 피해량 +25%, 쿨다운 감소 +6%", "[제작] \n레이피어 + 운명의 꽃", "희귀")
                up("듀랜달 Mk2", "볼틱레토", "유성검", "", "")
                down("레이피어", "운명의 꽃", "", "", "")
            }
            if (falselist[j].name1 == "활빈검") {
                te("활빈검", "공격력 +30, 시야 +1, 체력 재생+1.5, 스태미너 재생 +150%, 쿨다운 감소 +12% 치명타 피해량 +25%", "[제작] \n레이피어 + 어사의", "영웅")
                up("미스틸테인", "", "", "", "")
                down("레이피어", "어사의", "", "", "")
            }
            if (falselist[j].name1 == "듀랜달 Mk2") {
                te("듀랜달 Mk2", "공격력 +53, 시야 +1.5, 생명력 흡수 +8%, 치명타 확률 +12%, 치명타 피해량 +30%, 쿨다운 감소 +6%", "[제작]\n 매화검 + 레이저 포인터", "영웅")
                down("매화검", "레이저 포인터", "", "", "")
            }
            if (falselist[j].name1 == "볼틱레토") {
                te("볼틱레토", "공격력 +47, 공격속도 +45%, 치명타 확률 +10%, 치명타 피해량 +25%, 쿨다운 감소 +6%", "[제작]\n 매화검 + 전자 부품", "영웅")
                down("매화검", "전자 부품", "", "", "")
            }
            if (falselist[j].name1 == "유성검") {
                te("유성검", "공격력 +67, 치명타 피해량 +20%\n치명타 확률 +20%, 쿨다운 감소 +12%", "[제작]\n 매화검 + 운석", "영웅")
                down("매화검", "운석", "", "", "")
            }
            if (falselist[j].name1 == "주와이외즈") {
                te("주와이외즈", "공격력 +69, 치명타 피해량 +30%\n공격 속도 +12%, 이동속도 +0.2", "[제작]\n 레이피어 + 미스릴", "영웅")
                down("레이피어", "미스릴", "", "", "")
            }
            if (falselist[j].name1 == "미스틸테인") {
                te("미스틸테인", "공격력 +33, 시야 +1, 치명타 피해량 +25%, 쿨다운 감소 +15%, 스태미너 재생 +150%, 체력 재생 +1.5, 스킬 증폭 +17%", "[제작]\n 활빈검 + 나뭇가지", "전설")
                down("활빈검", "나뭇가지", "", "", "")
            }
            if (falselist[j].name1 == "단창") {
            ...
            if (falselist[j].name1 == "애각창") {
                te("애각창", "공격력 +105, 이동속도 +0.12", "[제작]\n 강창 + 백색 가루", "영웅")
                down("강창", "백색 가루", "", "", "")
            }
            if (falselist[j].name1 == "장팔사모") {
                te("장팔마소", "공격력 +100, 공격속도 +40%", "[제작]\n 강창 + 이온 전지", "영웅")
                down("강창", "이온 전지", "", "", "")
            }
            if (falselist[j].name1 == "코스믹 바이던트") {
                te("코스믹 바이던트", "공격력 +138", "[제작]\n 강창 + 문스톤", "영웅")
                down("강창", "문스톤", "", "", "")
            }
            if (falselist[j].name1 == "트리아이나") {
                te("트리아이나", "공격력 +90, 스킬 증폭 +21%, 쿨다운 감소 5%", "[제작]\n 바이던트 + 파이크", "영웅")
                down("바이던트", "파이크", "", "", "")
            }
            if (falselist[j].name1 == "방천화극") {
                te("방천화극", "공격력 +84, 이동속도 -0.07, 스킬 적중 시 치유 감소 -40%, 스킬 증폭 +31%", "[제작]\n 도끼창 + 군선", "영웅")
                down("도끼창", "군선", "", "", "")
            }
            if (falselist[j].name1 == "청룡언월도") {
                te("청룡언월도", "공격력 +118, 체력 +320, 이동속도 -0.05", "[제작]\n 도끼창 + 철판", "영웅")
                down("도끼창", "철판", "", "", "")
            }
            if (falselist[j].name1 == "화첨창") {
            ...
            if (falselist[j].name1 == "워해머") {
                te("워해머", "공격력 +40", "[제작]\n망치 + 단봉", "고급")
                up("모닝 스타", "사슴 망치", "마법봉", "", "")
                down("망치", "단봉", "", "", "")
            }
            if (falselist[j].name1 == "모닝 스타") {
                te("모닝 스타", "공격력 +55", "[제작]\n워해머 + 쇠구슬", "희귀")
                up("낭아봉", "개밥바라기", "", "", "")
                down("워해머", "쇠구슬", "", "", "")
            }
            ...
            if (falselist[j].name1 == "낭아봉") {
                te("낭아봉", "공격력 +55, 스킬 증폭 +33", "[제작]\n모닝 스타 + 달궈진 돌멩이", "영웅")
                down("모닝 스타", "달궈진 돌멩이", "", "", "")
            }
            if (falselist[j].name1 == "다그다의 망치") {
                te("다그다의 망치", "공격력 +95, 체력 재생 +200%, 기본공격 추가 피해 +22", "[제작]\n사슴 망치 + 성자의 유산", "영웅")
                down("사슴 망치", "성자의 유산", "", "", "")
            }
            if (falselist[j].name1 == "토르의 망치") {
            ...
            if (falselist[j].name1 == "마법봉") {
                te("마법봉", "공격력 +60, 생명력 흡수 +30%, 시야 +0.5, 스킬 증폭 +27%", "[제작]\n 워해머 + 달빛 펜던트", "영웅")
                down("워해머", "달빛 펜던트", "", "", "")
            }
            if (falselist[j].name1 == "쇠구슬") {
                te("쇠구슬", "장탄 수: 50, 공격력 +12", "양궁장(6개), 숲(7개), 공장(7개)의 상자\n" +
                        "맷돼지(사냥)", "일반")
                up("수류탄", "볼 라이트닝", "힐리스", "소란 발생기", "모닝 스타")
            }
            if (falselist[j].name1 == "야구공") {
            ...
            if (falselist[j].name1 == "가시 탱탱볼") {
                te("가시 탱탱볼", "공격력 +45, 스킬 적중 시 치유 감소 -40%", "[제작]\n플러버 + 못", "희귀")
                up("루테늄 구슬", "", "", "", "")
                down("플러버", "못", "", "", "")
            }
            if (falselist[j].name1 == "소이탄") {
                te("소이탄", "최대 충전 수 : 50발, 공격력 +66, 쿨다운 감소 +10%, 공격속도 +60%", "[제작]\n화염병 + 볼 라이트닝", "영웅")
                down("화염병", "볼 라이트닝", "", "", "")
            }
            if (falselist[j].name1 == "안티오크의 수류탄") {
                te("안티오크의 수류탄", "최대 충전 수 : 8발, 공격력 +77, 기본 공격 추가 피해 +95", "[제작]\n고폭 수류탄 + 십자가", "영웅")
                down("고폭 수류탄", "십자가", "", "", "")
            }
            if (falselist[j].name1 == "다비드슬링") {
                te("다비드슬링", "최대 충전 수 : 50발, 공격력 +68, 체력재생 +200%, 기본 공격 추가 피해 +33", "[제작]\n슬링 + 성자의 유산", "영웅")
                down("슬링", "성자의 유산", "", "", "")
            }
            if (falselist[j].name1 == "연막탄") {
                te("연막탄", "최대 충전 수 : 24발, 공격력 +85, 공격속도 +40%", "[제작]\n밀가루 폭탄 + 콜라", "영웅")
                down("밀가루 폭탄", "콜라", "", "", "")
            }
            if (falselist[j].name1 == "고폭 수류탄") {
                te("고폭 수류탄", "최대 충전 수 : 8발, 공격력 +56, 기본 공격 추가 피해 +60", "[제작]\n수류탄 + RDX", "영웅")
                down("수류탄", "RDX", "", "", "")
            }
            if (falselist[j].name1 == "루테늄 구슬") {
                te("루테늄 구슬", "최대 충전 수 : 50발, 공격력 +76, 스킬 증폭 +28%, 스킬 적중 시 치유 감소 -40%", "[제작]\n가시 탱탱볼 + 황금", "영웅")
                down("가시 탱탱볼", "황금", "", "", "")
            }

            if (falselist[j].name1 == "너클") {
                te("너클", "공격력 +10", "묘지(5개), 숲(5개)의 상자", "일반")
                up("아이언 너클", "", "", "", "")
            }
            if (falselist[j].name1 == "목장갑") {
            ...
            if (falselist[j].name1 == "단영촌천투") {
                te("단영촌천투", "공격력 +50, 방어력 +18, 스태미너 재생 +50%, 생명력 흡수 +12%", "[제작]\n귀골 장갑 + 인형", "영웅")
                up("", "", "", "", "")
                down("귀골 장갑", "인형", "", "", "")
            }

            if (falselist[j].name1 == "디바인 피스트") {
            ...
            if (falselist[j].name1 == "여래수투") {
                te("여래수투", "공격력 +43, 쿨다운 감소 +19% 스킬 증폭 +19%", "[제작]\n회단 장갑 + 진신사리", "영웅")
                up("", "", "", "", "")
                down("회단 장갑", "진신사리", "", "", "")
            }

            if (falselist[j].name1 == "브레이질 건틀릿") {
            ...
            if (falselist[j].name1 == "소수") {
                te("소수", "공격력 +58, 치명타 확률 +30%", "[제작]\n유리 너클 + 백색 가루", "영웅")
                up("", "", "", "", "")
                down("유리 너클", "백색 가루", "", "", "")
            }

            if (falselist[j].name1 == "천잠장갑") {
                te("천잠장갑", "공격력 +88, 이동속도 +0.22", "[제작]\n미스릴 실 + 글러브", "영웅")
                up("", "", "", "", "")
                down("미스릴 실", "글러브", "", "", "")
            }

            if (falselist[j].name1 == "대나무") {
            ...
            if (falselist[j].name1 == "택티컬 톤파") {
                te("택티컬 톤파", "공격력 +74, 방어력 +16, 생명력 흡수 +25%", "[제작]\n 경찰봉 + 정교한 도면", "영웅")
                down("경찰봉 + 정교한 도면", "", "", "", "")
            }
            if (falselist[j].name1 == "마이쏙") {
                te("마이쏙", "공격력 +63, 방어력 +25, 체력 재생 +100%", "[제작]\n 류큐톤파 + 단봉", "영웅")
                down("류큐톤파", "단봉", "", "", "")
            }
            ...
            if (falselist[j].name1 == "구원의 여신상") {
                te("구원의 여신상", "공격력 +78, 체력 재생 +0.6, 스태미너 재생 +80%, 공격 속도 +15%, 생명력 흡수 +12%", "[제작]\n횃불 + 인형", "영웅")
                up("", "", "", "", "")
                down("횃불", "인형", "", "", "")
            }
            if (falselist[j].name1 == "타구봉") {
                te("타구봉", "공격력 +85, 공격속도 +50%, 기본 공격 시 치유 감소 -40%", "[제작]\n도깨비 방망이 + 전자 부품", "영웅")
                up("", "", "", "", "")
                down("도깨비 방망이", "전자 부품", "", "", "")
            }
            if (falselist[j].name1 == "스파이의 우산") {
                te("스파이의 우산", "공격력 +65, 스킬 증폭 +30, 스태미너 재생 +0.8", "[제작]\n우산 + 재", "영웅")
                up("", "", "", "", "")
                down("우산", "재", "", "", "")
            }
            ...
            if (falselist[j].name1 == "편전") {
                te("편전", "공격력 +60, 기본 공격 추가 피해 +45", "[제작]\n국궁 + 대나무", "영웅")
                up("", "", "", "", "")
                down("국궁", "대나무", "", "", "")
            }

            if (falselist[j].name1 == "골든래쇼 보우") {
                te("골든래쇼 보우", "공격력 +68, 쿨다운 감소 +15%, 스킬 증폭 +20%", "[제작]\n탄궁 + 황금", "영웅")
                up("", "", "", "", "")
                down("탄궁", "황금", "", "", "")
            }

            if (falselist[j].name1 == "트윈보우") {
                te("트윈보우", "공격력 +70, 공격속도 +55%, 기본 공격 시 치유 감소 -40%, 시야 +1", "[제작]\n강궁 + 컴포지트 보우", "영웅")
                up("", "", "", "", "")
                down("강궁", "컴포지트 보우", "", "", "")
            }
            if (falselist[j].name1 == "엘리멘탈 보우") {
                te("엘리멘탈 보우", "공격력 +60, 이동속도 +0.13, 스킬 증폭 +38, 스킬 적중 시 치유 감소 -40%", "[제작]\n화전 + 백우선", "영웅")
                up("", "", "", "", "")
                down("화전", "백우선", "", "", "")
            }
            ...
            if (falselist[j].name1 == "대황") {
                te("대황", "공격력 +75, 체력 재생 +2, 공격 속도 +50%", "[제작]\n철궁 + 기름먹힌 천", "영웅")
                up("", "", "", "", "")
                down("철궁", "기름먹인 천", "", "", "")
            }
            if (falselist[j].name1 == "발리스타") {
                te("발리스타", "공격력 +115\n" +
                        "공격 속도(%) -8%", "[제작]\n헤비 크로스보우 + 단창", "영웅")
                up("", "", "", "", "")
                down("헤비 크로스보우", "단창", "", "", "")
            }
            if (falselist[j].name1 == "저격 크로스보우") {
                te("저격 크로스보우", "공격력 +85, 시야 +3\n" +
                        "기본 공격 사거리 +1", "[제작]\n저격궁 + 저격 스코프", "영웅")
                up("", "", "", "", "")
                down("저격궁", "저격 스코프", "", "", "")
            }
            if (falselist[j].name1 == "영광금귀신기노") {
                te("영광금귀신기노", "공격력 +66\n" +
                        "기본 공격 추가 피해 +44", "[제작]\n노 + RDX", "영웅")
                up("", "", "", "", "")
                down("노", "RDX", "", "", "")
            ...
            if (falselist[j].name1 == "면도칼") {
                ...
                for (j in falselist.indices) {
                    ...
                    if (falselist[j].name1 == "흑건") {
                        imaup2.setImageResource(falselist[j].Image1)
                        uptext2.text = falselist[j].name1
                    }
                    if (falselist[j].name1 == "치파오") {
                        imaup3.setImageResource(falselist[j].Image1)
                        uptext3.text = falselist[j].name1
                    }
                    if (falselist[j].name1 == "블리더") {
                        imaup4.setImageResource(falselist[j].Image1)
                        uptext4.text = falselist[j].name1
                    }
                    if (falselist[j].name1 == "철편") {
            ...
            if (falselist[j].name1 == "분필") {
                te("분필", "장탄 수: 50, 공격력 +12", "공장, 성당, 학교의 상자\n" +
                        "박쥐(사냥)", "일반")
                up("백색 가루", "", "", "", "")
                down("", "", "", "", "")
            }
            if (falselist[j].name1 == "트럼프 카드") {
            ...
            if (falselist[j].name1 == "만천화우") {
                te("만천화우", "장탄 수: 3, 공격력 +80, 치명타 피해량+50%, 스킬 증폭 +30, 기본 공격 추가 피해 +150", "[제작]\n빙백은침 + 폭뢰침", "전설")
                up("", "", "", "", "")
                down("빙백은침", "폭뢰침", "", "", "")
            }

            if (falselist[j].name1 == "부적") {
            ...
            if (falselist[j].name1 == "독침") {
                te("독침", "장탄 수: 3, 공격력 +55, 치명타 피해량 +20%, 스킬 증폭 +15", "[제작]\n바늘 + 독약", "희귀")
                up("빙백은침", "", "", "", "")
                down("바늘", "독약", "", "", "")
            }
            if (falselist[j].name1 == "법륜") {
            ...
            if (falselist[j].name1 == "미치광이왕의 카드") {
                te("미치광이왕의 카드", "장탄 수: 52, 공격력 +49, 치명타 피해 증가 +10%, 공격속도 +40%, 치명타 확률 +25%", "[제작]\n빈티지 카드 + 이온 전지", "영웅")
                up("", "", "", "", "")
                down("빈티지 카드", "이온 전지", "", "", "")
            }
            if (falselist[j].name1 == "옥전결") {
                te("옥전결", "장탄 수: 12, 공격력 +60, 스킬 증폭 +15", "[제작]\n부적 + 재", "영웅")
                up("", "", "", "", "")
                down("부적", "재", "", "", "")
            }
            if (falselist[j].name1 == "풍마 수리검") {
                te("풍마 수리검", "장탄 수: 50, 공격력 +58, 쿨다운 감소 +15%, 기본 공격 추가 피해 +40", "[제작]\n매화비표 + 알코올", "영웅")
                up("", "", "", "", "")
                down("매화비표", "알코올", "", "", "")
            }
            if (falselist[j].name1 == "빙백은침") {
                te("빙백은침", "장탄 수: 3, 공격력 +80, 스킬 증폭 +20, 치명타 피해량 +50%", "[제작]\n독침 + 얼음", "영웅")
                up("만천화우", "", "", "", "")
                down("독침", "얼음", "", "", "")
            }
            if (falselist[j].name1 == "푸른색 단도") {
                te("푸른색 단도", "장탄 수: 50, 공격력 +20, 기본 공격 추가 피해 +33, 스킬 증폭 +24", "[제작]\n흑건 + 독약", "영웅")
                up("", "", "", "", "")
                down("흑건", "독약", "", "", "")
            }
            ...
            if (falselist[j].name1 == "건곤권") {
                te("건곤권", "장탄 수: 50, 공격력 +51, 스킬 증폭 +23%", "[제작]\n법륜 + 대나무", "영웅")
                up("", "", "", "", "")
                down("법륜", "대나무", "", "", "")
            }
            if (falselist[j].name1 == "생사부") {
                te("생사부", "장탄 수: 15, 공격력 +80, 스킬 증폭 +20%", "[제작]\n부적 + 생명의 나무", "영웅")
                up("", "", "", "", "")
                down("부적", "생명의 나무", "", "", "")
            }
            if (falselist[j].name1 == "보급형 기타") {
            ...
            if (falselist[j].name1 == "험버커 픽업") {
                te("험버커 픽업", "공격력 +25, 시야 +3, 스킬 증폭 +15%", "[제작]\n골든 브릿지 + 저격 스코프", "희귀")
                up("천국의 계단", "", "", "", "")
                down("골든 브릿지", "저격 스코프", "", "", "")
            }
            if (falselist[j].name1 == "King-V") {
                te("King-V", "공격력 +13, 스킬증폭 +16%", "[제작]\n골든 브릿지 + 가위", "희귀")
                up("퍼플 헤이즈", "", "", "", "")
                down("골든 브릿지", "가위", "", "", "")
            }
            if (falselist[j].name1 == "노캐스터") {
            ...
            if (falselist[j].name1 == "새티스팩션") {
                te("새티스팩션", "공격력 +30, 공격속도 +30%, 생명력 흡수 +15%, 기본 공격 추가 피해 +13", "[제작]\n노캐스터 + 돌멩이", "영웅")
                up("", "", "", "", "")
                down("노캐스터", "돌멩이", "", "", "")
            }
            if (falselist[j].name1 == "원더풀 투나잇") {
            ...
            if (falselist[j].name1 == "틴 스피릿") {
                te("틴 스피릿", "공격력 +10, 기본 공격 사거리 +1, 공격속도 +30%, 기본 공격 추가 피해 +22", "[제작]\n야생마 + 진신사리", "영웅")
                up("", "", "", "", "")
                down("야생마", "진신사리", "", "", "")
            }
            if (falselist[j].name1 == "쇠사슬") {
                name.text = "쇠사슬"
                abil.text = "공격력 +13"
                exeplan.text = "사용 시 기본 지급\n" +
            ...
            if (falselist[j].name1 == "방탄모") {
                te("방탄모", "방어력 +15, 최대 체력 +90, 쿨다운 감소 +8%", "[제작] 베레모 + 자전거 헬멧\n파란색 항공 보급", "희귀")
                up("전술-OPS 헬멧", "", "", "", "")
                down("베레모", "자전거 헬멧", "", "", "")
            }
            if (falselist[j].name1 == "소방 헬멧") {
                te("소방 헬멧", "최대 체력 +250", "[제작] 안전모 + 물\n파란색 항공 보급", "희귀")
                up("오토바이 헬멧", "", "", "", "")
                down("안전모", "물", "", "", "")
            }
            if (falselist[j].name1 == "티아라") {
            ...
            if (falselist[j].name1 == "수정 티아라") {
                te("수정 티아라", "방어력 +21, 최대 스태미너 +600, 치명타 확률 +22%", "[제작]티아라 + 유리 조각\n", "영웅")
                up("", "", "", "", "")
                down("티아라", "유리 조각", "", "", "")
            }
            ...
            if (falselist[j].name1 == "제국 왕관") {
                te("제국 왕관", "방어력 +20, 최대 체력 +330, 스킬 증폭 +10%", "[제작] 왕관 + 루비\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("왕관", "루비", "", "", "")
            }
            ...
            if (falselist[j].name1 == "방탄조끼") {
                te("방탄조끼", "방어력 +10, 최대 체력 +130, 기본 공격 피해 감소 +10", "[제작] 군복 + 철판\n파란색 항공 보급", "희귀")
                up("배틀 슈트", "EOD 슈트", "", "", "")
                down("군복", "철판", "", "", "")
            }
            if (falselist[j].name1 == "석양의 갑옷") {
                te("석양의 갑옷", "방어력 +20, 최대 체력 +360", "[제작] 가죽 갑옷 + 루비\n파란색 항공 보급", "희귀")
                up("", "", "", "", "")
                down("가죽 갑옷", "루비", "", "", "")
            }
            if (falselist[j].name1 == "어사의") {
                te("어사의", "방어력 +23, 시야 +1, 최대 체력 +2.5, 쿨다운 감소 +15%", "[제작] 한복 + 마패\n", "희귀")
                up("활빈검", "", "", "", "")
                down("한복", "마패", "", "", "")
            }
            if (falselist[j].name1 == "광학미채 슈트") {
                te("광학미채 슈트", "공격력 +10, 방어력 +18, 치명타 확률 +22%, 스킬 피해 감소 +10%", "[제작] 잠수복 + 유리판\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("잠수복", "유리판", "", "", "")
            }
            if (falselist[j].name1 == "락커의 자켓") {
                te("락커의 자켓", "공격력 +30, 방어력 +18, 비전투중 이동속도 +0.1, 기본 공격 피해 감소 +15, 스킬 적중 시 치유 감소 -40% (원거리 25%)", "[제작] 라이더 자켓 + 소란 발생기\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("라이더 자켓", "소란 발생기", "", "", "")
            }
            if (falselist[j].name1 == "미스릴 갑옷") {
                te("미스릴 갑옷", "방어력 +60, 공격속도 +12%, 이동속도 +0.2, 스킬 피해 고정 감소 +14", "[제작] 미스릴 + 가죽 갑옷\n보라색 항공 보급", "미스릴 갑옷")
                up("", "", "", "", "")
                down("미스릴", "가죽 갑옷", "", "", "")
            }
            if (falselist[j].name1 == "성기사의 갑옷") {
                te("성기사의 갑옷", "방어력 +38, 체력 재생 +150%, 기본 공격 추가 피해 +15", "[제작] 판금 갑옷 + 성자의 유산\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("판금 갑옷", "성자의 유산", "", "", "")
            }
            if (falselist[j].name1 == "아마조네스 아머") {
                te("아마조네스 아머", "방어력 +28, 최대 체력 +140, 스킬 피해 감소 +20%", "[제작] 판금 갑옷 + 비키니\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("판금 갑옷", "비키니", "", "", "")
            }
            if (falselist[j].name1 == "용의 도복") {
                te("용의 도복", "공격력 +11, 방어력 +10, 스태미너 재생 +120%, 쿨다운 감소 +10%, 기본 공격 사거리 +1", "[제작] 치파오 + 거북 도복\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("치파오", "거북 도복", "", "", "")
            }
            if (falselist[j].name1 == "지휘관의 갑옷") {
                te("지휘관의 갑옷", "방어력 +24, 스킬 증폭 +18%", "[제작] 사슬 갑옷 + 황금\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("사슬 갑옷", "황금", "", "", "")
            }
            if (falselist[j].name1 == "집사복") {
                te("집사복", "공격력 +30, 이동속도 +0.2, 생명력 흡수 +20%, 기본 공격 피해 감소 +10", "[제작] 정장 + 먼지털이개\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("정장", "먼지털이개", "", "", "")
            }
            if (falselist[j].name1 == "배틀 슈트") {
                te("배틀 슈트", "방어력 +25, 최대 체력 +170, 기본 공격 피해 감소 +15, 스킬 피해 감소 +12%", "[제작] 방탄조끼 + 잠수복\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("방탄조끼", "잠수복", "", "", "")
            }
            if (falselist[j].name1 == "EOD 슈트") {
                te("EOD 슈트", "방어력 +25, 최대 체력 +250, 체력 재생 +1, 쿨다운 감소 +10%, 기본 공격 피해 감소 +10", "[제작] 방탄조끼 + 덧댄 로브\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("방탄조끼", "덧댄 로브", "", "", "")
            }
            if (falselist[j].name1 == "불꽃 드레스") {
                te("불꽃 드레스", "방어력 +30, 공격력 +20, 체력 재생 +2, 스태미너 재생 +2, 스태미너 재생 +75%, 기본 공격 추가 피해 +20", "[제작] 삼매진화 + 드레스\n노란색 항공 보급", "전설")
                up("", "", "", "", "")
                down("삼매진화", "드레스", "", "", "")
            }
            if (falselist[j].name1 == "카바나") {
            ...
            if (falselist[j].name1 == "큐브 워치") {
                te("큐브 워치", "방어력 +22, 공격속도 +50%", "[제작] 고장난 시계 + 문스톤\n보란색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("고장난 시계", "문스톤", "", "", "")
            }
            ...
            if (falselist[j].name1 == "바이탈 센서") {
                te("바이탈 센서", "방어력 +15, 공격속도 +30%", "[제작] 손목 시계 + 휴대폰\n보라색 항공 보급", "영웅")
                up("레이더", "", "", "", "")
                down("손목 시계", "휴대폰", "", "", "")
            }
            if (falselist[j].name1 == "기사의 신조") {
                te("기사의 신조", "공격력 +30, 방어력 +20, 체력 재생 +1.5, 이동속도 -0.03, 치명타 피해량 +25%", "[제작] 강철 방패 + 분대장 완장\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("강철 방패", "분대장 완장", "", "", "")
            }
            ...
            if (falselist[j].name1 == "캐리비안 장식총") {
                te("캐리비안 장식총", "공격력 +10, 생명력 흡수 +6%", "[제작] 발터 PPK + 리본\n 초록색 항공 보급", "고급")
                up("해적의 증표", "", "", "", "")
                down("발터 PPK", "리본", "", "", "")
            }
            if (falselist[j].name1 == "해적의 증표") {
                te("해적의 증표", "공격력 +15, 생명력 흡수 +6%, 치명타 확률 +12%", "[제작] 캐리비안 장식총 + 식칼\n파라색 항공 보급", "희귀")
                up("", "", "", "", "")
                down("캐리비안 장식총", "식칼", "", "", "")
            }
            if (falselist[j].name1 == "아이기스") {
                te("아이기스", "체력 +320, 스태미너 +250, 공격력 +8, 방어력 +12", "[제작] 진홍 팔찌 + 가죽 방패\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("진홍 팔찌", "가죽 방패", "", "", "")
            }
            if (falselist[j].name1 == "스카디의 팔찌") {
                te("스카디의 팔찌", "최대 체력 +200, 최대 스태미너 +250, 체력 재생 +1.5, 스태미너 재생 +1.5, 스킬 증폭 +24", "[제작] 만년빙 + 진홍 팔찌\n노란색 항공 보급", "전설")
                up("", "", "", "", "")
                down("만년빙", "진홍 팔찌", "", "", "")
            }
            if (falselist[j].name1 == "레이더") {
                te("레이더", "방어력 +18, 공격속도 +40%, 치명타 확률 +24%", "[제작] 바이탈 센서 + 유리판\n노란색 항공 보급", "전설")
                up("", "", "", "", "")
                down("바이탈 센서", "유리판", "", "", "")
            }
            ...
            if (falselist[j].name1 == "매버릭 러너") {
                te("매버릭 러너", "공격속도 +5%, 이동속도 +0.2, 비전투중 이동속도 +0.45", "[제작] 힐리스 + 전자 부품\n파란색 항공 보급", "희귀")
                up("", "", "", "", "")
                down("힐리스", "전자 부품", "", "", "")
            }
            ...
            if (falselist[j].name1 == "킬힐") {
                te("킬힐", "공격력 +5, 스태미너 재생 +0.8, 이동속도 +0.16, 치명타 확률 +10%", "[제작] 하이힐 + 유리 조각\n파란색 항공 보급", "희귀")
                up("분홍신", "부케팔로스", "", "", "")
                down("하이힐", "유리 조각", "", "", "")
            }
            if (falselist[j].name1 == "풍화륜") {
                te("풍화륜", "스태미너 재생 +0.5, 스태미너 재생 +50%, 이동속도 +0.33, 스킬증폭 +10", "[제작] 덧댄 슬리퍼 + 재\n파란색 항공 보급", "희귀")
                up("", "", "", "", "")
                down("덧댄 슬리퍼", "재", "", "", "")
            }
            ...
            if (falselist[j].name1 == "미스릴 부츠") {
                te("미스릴 부츠", "방어력 +22, 공격속도 +12%, 이동속도 +0.5", "[제작] 미스릴 + 체인 레깅스\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("미스릴", "체인 레깅스", "", "", "")
            }
            if (falselist[j].name1 == "부케팔로스") {
                te("부케 팔로스", "공격력 +10, 방어력 +10, 스태미너 재생 +1, 이동속도 +0.33, 치명타 확률 +15%", "[제작]킬힐 + 체인 레깅스\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("킬힐", "체인 레깅스", "", "", "")
            }
            if (falselist[j].name1 == "클링온 부츠") {
                te("클링온 부츠", "체력 재생 +0.7, 공격속도 +15, 이동속도 +0.35, 기본 공격 시 치유 감소 -40% (원거리 25%)", "[제작] 전투화 + 못\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("전투화", "못", "", "", "")
            }
            if (falselist[j].name1 == "EOD 부츠") {
                te("EOD 부츠", "방어력 +26, 체력 재생 +1, 스태미너 재생 +0.5, 이동속도 +0.33", "[제작] 부츠 + 강철 무릎 보호대\n노란색 항공 보급", "전설")
                up("", "", "", "", "")
                down("부츠", "강철 무릎 보호대", "", "", "")
            }
            if (falselist[j].name1 == "글레이셜 슈즈") {
                te("글레이셜 슈즈", "체력 재생 +1.5, 스태미너 재생 +1.5, 스태미너 재생 +60%, 이동속도 +0.25, 비전투중 이동속도 +0.4, 스킬증폭 +20", "[제작] 덧댄 슬리퍼 + 만년빙\n노란색 항공 보급", "전설")
                up("", "", "", "", "")
                down("만년빙", "덧댄 슬리퍼", "", "", "")
            }
            ...
            if (falselist[j].name1 == "슈뢰딩거의 상자") {
                te("슈뢰딩거의 상자", "최대 체력 +360, 스킬 증폭 +7", "[제작] 상자 + 독약\n파란색 항공 보급", "희귀")
                up("", "", "", "", "")
                down("상자", "독약", "", "", "")
            }
            if (falselist[j].name1 == "진리는 나의 빛") {
            ...
            if (falselist[j].name1 == "에메랄드 타블렛") {
                te("에메랄드 타블렛", "공격력 +15, 방어력 +15, 치명타 확률 +15%, 쿨다운 감소 +10%", "[제작] 포스 코어 + 운명의 꽃\n노라색 항공 보급", "전설")
                up("", "", "", "", "")
                down("포스 코어", "운명의 꽃", "", "", "")
            }
            if (falselist[j].name1 == "비파단도") {
                te("비파단도", "공격력 +5, 스킬증폭 +8%", "[제작]\n녹슨 검 + 마패", "고급")
                up("월왕구천", "", "", "", "")
                down("녹슨 검", "마패", "", "", "")
            }
            if (falselist[j].name1 == "월왕구천") {
                te("월왕구천", "공격력 +15, 스킬증폭 20%", "[제작]\n비파단도 + 정교한 도면", "희귀")
                up("", "", "", "", "")
                down("비파단도", "정교한 도면", "", "", "")
            }
            if (falselist[j].name1 == "요명월") {
                te("요명월", "방어력 +11, 스태미너 재생 +160%, 고정 스킬 피해 감소 +18, 시야 +1.5", "[제작]\n정화수 + 문스톤", "영웅")
                up("", "", "", "", "")
                down("정화수", "문스톤", "", "", "")
            }
            if (falselist[j].name1 == "천사의 고리") {
                te("천사의 고리", "공격력 +40, 방어력 +30, 최대 스태미너 +300, 치명타 피해량 감소 +10%", "[제작]\n머리테 + 포스 코어", "전설")
                up("", "", "", "", "")
                down("머리테", "포스 코어", "", "", "")
            }

            if (falselist[j].name1 == "감자") {
                te("감자", "체력 재생 +130", "감자 덩굴에서 채집", "일반")
                up("감자빵", "감자스프", "감자튀김", "구운 감자", "버터 감자구이")
                down("", "", "", "", "")
            }
            if (falselist[j].name1 == "대구") {
                te("대구", "체력 재생 +130", "바다 낚시(모래사장, 고급 주택가, 항구)에서 채집", "일반")
                up("꿀 바른 대구살", "대구 간 통조림", "달걀 생선 필레", "메로구이", "생선까스")
                down("", "", "", "", "")
            }
            if (falselist[j].name1 == "레몬") {
                te("레몬", "체력 재생", "호텔(6개), 고급 주택가(5개), 병원(4개)의 상자", "일반")
                up("레몬에이드", "칵테일", "시트러스 케이크", "레몬 아이스크림", "")
                down("", "", "", "", "")
            }
            if (falselist[j].name1 == "마늘") {
                te("마늘", "체력 재생 +200", "골목길(6개), 절(6개), 묘지(5개)의 상자", "일반")
                up("마늘빵", "마늘 베이컨 말이", "마늘 꿀절임", "마늘라면", "")
                down("", "", "", "", "")
            }
            if (falselist[j].name1 == "반창고") {
                te("반창고", "체력 재생 +320", "병원(4개)의 상자", "일반")
                up("지혈제", "", "", "", "")
                down("", "", "", "", "")
            }
            if (falselist[j].name1 == "붕어") {
                te("붕어", "체력 재생 +130", "민물 낚시에서 채집", "일반")
                up("붕어빵", "매운탕", "구운 붕어", "", "")
                down("", "", "", "", "")
            }
            if (falselist[j].name1 == "빵") {
                name.text = "빵"
                abil.text = "체력 재생 +275"
            ...
            if (falselist[j].name1 == "성수") {
                te("성수", "체력 재생 +600", "초록색 항공 보급", "고급")
                up("", "", "", "", "")
                down("", "", "", "", "")
            }
            if (falselist[j].name1 == "지혈제") {
            ...
            if (falselist[j].name1 == "탄두리") {
                te("탄두리", "체력 재생 +600", "[제작] 카레 가루 + 고기\n초록색 항공 보급", "고급")
                up("", "", "", "", "")
                down("카레 가루", "고기", "", "", "")
            }
            if (falselist[j].name1 == "마늘 베이컨 말이") {
                te("마늘 베이컨 말이", "체력 재생 +650", "[제작] 마늘 + 고기\n초록색 항공 보급", "고급")
                up("", "", "", "", "")
                down("마늘", "고기", "", "", "")
            }
            if (falselist[j].name1 == "번") {
                te("번", "체력 재생 +650", "[제작] 빵 + 커피콩\n초록색 항공 보급", "고급")
                up("", "", "", "", "")
                down("빵", "커피콩", "", "", "")
            }
            if (falselist[j].name1 == "햄버거") {
            ...
            if (falselist[j].name1 == "카레 고로케") {
                te("카레 고로케", "체력 재생 +600", "[제작] 카레 가루 + 뜨거운 오일\n파란색 항공 보급", "희귀")
                up("", "", "", "", "")
                down("카레 가루", "뜨거운 오일", "", "", "")
            }
            ...
            if (falselist[j].name1 == "구급상자") {
                te("구급상자", "체력 재생 +950", "[제작] 지혈제 + 붕대\n파란색 항공 보급", "희귀")
                up("", "", "", "", "")
                down("지혈제", "붕대", "", "", "")
            }
            if (falselist[j].name1 == "버터 감자구이") {
            ...
            if (falselist[j].name1 == "볶음라면") {
                te("볶음라면", "체력 재생 +600", "[제작] 생라면 + 뜨거운 오일\n파란색 항공 보급", "희귀")
                up("", "", "", "", "")
                down("생라면", "뜨거운 오일", "", "", "")
            }
            if (falselist[j].name1 == "냉면") {
                te("냉면", "체력 재생 +650", "[제작] 생라면 + 얼음물\n파란색 항공 보급", "희귀")
                up("", "", "", "", "")
                down("생라면", "얼음물", "", "", "")
            }
            if (falselist[j].name1 == "대환단") {
                te("대환단", "체력 재생 +900", "[제작]한방침 + 보약\n파란색 항공 보급", "희귀")
                up("", "", "", "", "")
                down("한방침", "보약", "", "", "")
            }
            if (falselist[j].name1 == "마늘라면") {
                te("마늘라면", "체력 재생 +700", "[제작] 뜨거운 라면 + 마늘\n파란색 항공 보급", "희귀")
                up("", "", "", "", "")
                down("뜨거운 라면", "마늘", "", "", "")
            }
            if (falselist[j].name1 == "피쉬 앤 칩스") {
                te("피쉬 앤 칩스", "체력 재생 +920", "[제작]생선까스 + 감자튀김\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("생선까스", "감자튀김", "", "", "")
            }
            if (falselist[j].name1 == "꿀") {
                name.text = "꿀"
            ...
            if (falselist[j].name1 == "얼음") {
                name.text = "얼음"
                abil.text = "스태미너 재생 +200"
                exeplan.text = "호텔(7개), 병원(7개), 묘지(7개)의 상자"
                grade.text = "일반"
                im.text = "상위 아이템1 (터치 시 상위 아이템2로 변경)"
                for (j in falselist.indices) {
                    if (falselist[j].name1 == "만년빙") {
            ...
            if (falselist[j].name1 == "미스릴 실") {
                te("미스릴 실", "트랩 피해 +285", "[제작] 미스릴 + 피아노선\n파란색 항공 보급", "희귀")
                up("천잠장갑", "", "", "", "")
                down("피아노선", "미스릴", "", "", "")
            }
            if (falselist[j].name1 == "화염 트랩") {
                te("화염 트랩", "트랩 발동: 1.5초 후 넓은 범위로 폭발합니다.\n" +
                        "트랩 피해 +265", "[제작] 폭발 트랩 + 기름먹인 천\n파란색 항공 보급", "희귀")
                up("", "", "", "", "")
                down("폭발 트랩", "기름먹인 천", "", "", "")
            }
            if (falselist[j].name1 == "히든 메이든") {
                te("히든 메이든", "트랩 발동: 대상의 이동속도가 2초간 30% 감소합니다.\n" +
                        "트랩 피해 +285", "[제작] 가시 발판 + 정글 기요틴\n파란색 항공 보급", "희귀")
                up("", "", "", "", "")
                down("가시 발판", "정글 기요틴", "", "", "")
            }
            if (falselist[j].name1 == "폭뢰침") {
            ...
            if (falselist[j].name1 == "더블 기요틴") {
                te("더블 기요틴", "트랩 발동: 대상을 1초 간 속박합니다.\n" +
                        "트랩 피해 +260", "[제작] 정글 기요틴 + 펜듈럼 도끼\n", "영웅")
                up("", "", "", "", "")
                down("정글 기요틴", "펜듈럼 도끼", "", "", "")
            }
            if (falselist[j].name1 == "크레모어") {
                te("크레모어", "트랩 발동: 좁은 범위로 폭발하며, 피격된 대상은 1초간 기절합니다.\n" +
                        "트랩 피해 +240", "[제작] 지뢰 + 폭발 트랩\n보라색 항공 보급", "영웅")
                up("", "", "", "", "")
                down("지뢰", "항공 보급", "", "", "")
            }
            ...
            if (falselist[j].name1 == "라이터") {
                name.text = "라이터"
                abil.text = ""
                exeplan.text = "항구(9개), 골목길(8개), 공장(8개), 학교(8개)의 상자\n" +
                        "들개(가끔)를 처치"
                grade.text = "일반"
                im.isClickable = true
                im.text = "상위 아이템1 (터치 시 상위 아이템2로 변경)"
            ...
            if (falselist[j].name1 == "마패") {
                te("마패", "", "모래사장(6개), 골목길(6개), 절(7개)의 상자\n" +
                        "맷돼지(가끔)를 처치", "일반")
            ...
            if (falselist[j].name1 == "배터리") {
                te("배터리", "", "항구(8개), 번화가(9개), 공장(9개)의 상자\n" +
                        "들개(가끔)를 처치", "일반")
                up("방전 전지", "전자 부품", "", "", "")
                down("", "", "", "", "")
            }
            if (falselist[j].name1 == "알코올") {
            ...
            if (falselist[j].name1 == "접착제") {
                te("접착제", "", "골목길(6개), 병원(7개), 공장(8개)의 상자\n" +
                        "박쥐(가끔)를 처치", "일반")
            ...
            if (falselist[j].name1 == "강철") {
                ...
                im.setOnClickListener() {
                    ...
                    if (change == 1) {
                        ...
                        im.text = ("상위 아이템2 (터치 시 상위 아이템3 변경)")
                    }
                    else if(change == 2) {
                        for (j in falselist.indices) {
                            if (falselist[j].name1 == "95식 자동소총") {
                            ...
                            imaup5.setImageResource(0)
                            uptext5.text = ""
                            im.text = ("상위 아이템2 (터치 시 상위 아이템1 변경)")
                        }
                    }
                    else{
                        change = 0
        ...
        val dlg2 = Dialog(parent.context)
        ...
        return ViewHolder(view).apply{
            image.setOnClickListener {
                ...
                if (cursor == 0) {
                    ...
                    im1.setImageResource(R.drawable.a_chicken)
                    name.text = "닭"
                    randomte1.text = "고기"
                    randomte2.text = "가죽"
                    time.text = "0초"
                    location.text = "항구(4), 고급 주택가(5), 골목길(8), 호텔(3), 번화가(5), 병원(3), 공장(3), 성당(8), 학교(6)"
                    randomtitle.text = "랜덤 드랍"
                }
                if (cursor == 1){
                    im1.setImageResource(R.drawable.a_bat)
                    name.text = "박쥐"
                    randomtitle.text = "랜덤 드랍 [ 터치 시 다음 페이지 ]"
                    time.text = "60초"
                    location.text = "연못(4), 모래사장(3), 절(4), 양궁장(3), 묘지(4), 숲(3), 성당(3), 학교(4)"
                    random("가죽", "감시 카메라", "쌍안경", "성배", "불경")
                    ...
                    randomtitle.setOnClickListener {
                        ...
                        if(qkRnjtek == 0){
                            random("고철","레이저 포인터","못", "거북이 등딱지","고무")
                            randomtitle.text = "랜덤 드랍2 [ 터치 시 다음 페이지 ]"
                        }
                        if(qkRnjtek == 1){
                            random("상자","곡괭이","바늘","머리띠","가위")
                            randomtitle.text = "랜덤 드랍3 [ 터치 시 다음 페이지 ]"
                        }
                        if(qkRnjtek == 2){
                            random("모자","오일","분필","접착제","종이")
                            randomtitle.text = "랜덤 드랍4 [ 터치 시 다음 페이지 ]"
                        }
                        if(qkRnjtek == 3){
                            random("슬리퍼","붕대","면도칼","","")
                            randomtitle.text = "랜덤 드랍5 [ 터치 시 처음 페이지 ]"
                        }
                    }
                }
                if(cursor == 2){

                    im1.setImageResource(R.drawable.a_boar)
                    name.text = "멧돼지"
                    randomtitle.text = "랜덤 드랍 [ 터치 시 다음 페이지 ]"
                    time.text = "60초"
                    skill.text = "돌진 : 일직선으로 돌격하여 부딪힌 적을 밀치고 피해를 입힙니다. 벽에 부딪히면 기절합니다."
                    location.text = "연못(4), 모래사장(3), 절(3), 양궁장(3), 묘지(4), 숲(4), 성당(3), 번화가(6)"
                    random("만년필", "감시 카메라", "올가미", "쥐덫", "피아노선")
                    ...
                    randomtitle.setOnClickListener {
                        ...
                        if(qkRnjtek == 0){
                            random("타이즈","단창","마패", "승복","알코올")
                            randomtitle.text = "랜덤 드랍2 [ 터치 시 다음 페이지 ]"
                        }
                        if(qkRnjtek == 1){
                            random("자전거 헬멧","옷감","꽃","성배","식칼")
                            randomtitle.text = "랜덤 드랍3 [ 터치 시 다음 페이지 ]"
                        }
                        if(qkRnjtek == 2){
                            random("철광석","캔","쇠사슬","쇠구슬","유리병")
                            randomtitle.text = "랜덤 드랍4 [ 터치 시 다음 페이지 ]"
                        }
                        if(qkRnjtek == 3){
                            random("망치","트럼프 카드","깃털","","")
                ...
                if(cursor == 3){
                    ...
                    im1.setImageResource(R.drawable.a_dog)
                    name.text = "들개"
                    randomtitle.text = "랜덤 드랍 [ 터치 시 다음 페이지 ]"

                    time.text = "60초"

                    skill.text = "출혈: 들개의 기본 공격에 적중당한 적은 주기적인 추가 피해를 받는 출혈 상태에 걸립니다. 이 효과는 중첩 됩니다(최대 5회)"
                    location.text = "항구(3), 고급 주택가(3), 호텔(4), 번화가(4), 병원(3), 공장(4), 학교(4)"
                    random("손도끼", "레이저 포인터", "감시 카메라", "단봉", "바람막이")
                    ...
                    randomtitle.setOnClickListener {
                        ...
                        if(qkRnjtek == 0){
                            random("배터리","천 갑옷","전신 수영복", "손목시계","팔찌")
                            randomtitle.text = "랜덤 드랍2 [ 터치 시 다음 페이지 ]"
                        }
                        if(qkRnjtek == 1){
                            random("운동화","원석","리본","부채","불경")
                            randomtitle.text = "랜덤 드랍3 [ 터치 시 다음 페이지 ]"
                        }
                        if(qkRnjtek == 2){
                            random("십자가","쌍안경","고철","쇠구슬","유리병")
                            randomtitle.text = "랜덤 드랍4 [ 터치 시 처음 페이지 ]"
                        }
                    }
                }
                if(cursor == 4){
                    ...
                    randomtitle.text = "랜덤 드랍 [ 터치 시 다음 페이지 ]"
                    time.text = "120초"
                    skill.text = "소집: 반경 20m의 늑대들을 불러모아서 함께 적을 공격합니다."
                    location.text = "골목길(4), 호텔(4), 병원(4), 양궁장(4), 묘지(2), 숲(2), 공장(4)"
                    random("강철", "기름먹인 천", "뜨거운 오일", "루비", "방전 전지")
                    ...
                    randomtitle.setOnClickListener {
                        ...
                        if(qkRnjtek == 0){
                            random("백색 가루","운석","재", "전자 부품","정교한 도면")
                            randomtitle.text = "랜덤 드랍2 [ 터치 시 다음 페이지 ]"
                        }
                        if(qkRnjtek == 1){
                            random("철판","황금","먼지털이개","문스톤","독약")
                            randomtitle.text = "랜덤 드랍3 [ 터치 시 다음 페이지 ]"
                        }
                        if(qkRnjtek == 2){
                            random("모터","미스릴","유리판","이온 전지","")
                            randomtitle.text = "랜덤 드랍4 [ 터치 시 처음 페이지 ]"
                        }
                    }
                }
                if(cursor == 5){
                    ...
                    randomtitle.text = "랜덤 드랍 [ 터치 시 다음 페이지 ]"
                    time.text = "180초"
                    skill.text = "지면 강타: 바닥을 내리쳐 적에게 큰 피해를 입히고 기절시킵니다."
                    location.text = "항구(3), 연못(2), 모래사장(3), 고급 주택가(2), 골목길(3), 절(2)"
                    random("VF 혈액 샘플", "강철", "백색 가루", "루비", "전자 부품")
                    ...
                    randomtitle.setOnClickListener {
                        ...
                        if(qkRnjtek == 0){
                            random("정교한 도면","철판","독약", "모터","유리판")
                            randomtitle.text = "랜덤 드랍2 [ 터치 시 다음 페이지 ]"
                        }
                        if(qkRnjtek == 1){
                            random("이온 전지","기름먹인 천","뜨거운 오일","방전 전지","운석")
                            randomtitle.text = "랜덤 드랍3 [ 터치 시 다음 페이지 ]"
                        }
                        if(qkRnjtek == 2){
                            random("문스톤","황금","미스릴","포스 코어","")
                            randomtitle.text = "랜덤 드랍4 [ 터치 시 처음 페이지 ]"
                        }
                    }
                }
                ...
                drop1.setOnClickListener {
                    ...
                    for(i in falselist.indices){
                        ...
                        if(bitmap == bitmap2)
                        {
                            ImageView1.setImageResource(falselist[i].Image1)
                            ima_tex_clear()
                            falsearray(i)
    ...
    override fun onBindViewHolder(holder: animal_rv.ViewHolder, position: Int) {
        holder.image.setImageResource(animalItem.get(position).ani_ima)
        holder.name.text = animalItem.get(position).name
    }

    override fun getItemCount(): Int {
        return animalItem.size
    }

    class ViewHolder(itemView: View):RecyclerView.ViewHolder(itemView){
        val image = itemView.findViewById<ImageView>(R.id.animal1414)
        val name = itemView.findViewById<TextView>(R.id.ani_name)
    }
}
class animal_Item_list(val ani_ima:Int, val name:String){
